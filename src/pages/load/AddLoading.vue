    ...
    <div class="q-pa-md q-gutter-md">
      <q-list bordered class="flex column justify-center">
        ...
        <q-item clickable active-class="bg-grey-9 text-white">
          ...
          <q-item-section>
            <q-item-lable> {{ shipment.dispatch_number }} </q-item-lable>
          </q-item-section>
        </q-item>
        <q-separator />
      </q-list>
      ...
      >
        ...
        <q-tab-panel name="detail">
          ...
          >
            <q-item-label header> {{ shipment.dispatch_number }} </q-item-label>
            <q-separator inset />
            <q-item>
...
const fectSingleShipment = async (id) => {
  try {
    ...
    const shipmentById = await shipmentStore.fetchDispatchById(id);
...
const updateShipment = async () => {
  ...
  try {
    ...
    await pickingStore.handleCreateLoading(route.params.id, data);
