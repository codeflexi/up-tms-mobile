<template>
  <q-layout id="mainLayout" class="font-prompt-300">
    <q-header class="bg-red-14">
      <q-toolbar>
        <q-btn
          class="desktop-menu"
          flat
    ...
    <q-drawer v-model="leftDrawerOpen" show-if-above :width="250">
      <q-scroll-area id="menu-section">
        <q-list padding>
          <q-item clickable v-ripple to="/pickup">
            <q-item-section avatar>
              <q-icon name="pickup" />
            </q-item-section>
            <q-item-section> PICKUP </q-item-section>
          </q-item>

          <q-item clickable v-ripple to="/direct">
            <q-item-section avatar>
              <q-icon name="direct" />
            </q-item-section>
          ...
          <q-item clickable v-ripple to="/dispatch">
            <q-item-section avatar>
              <q-icon name="dispatch" />
            </q-item-section>

            <q-item-section> DISPATCH </q-item-section>
          </q-item>
          ...
          <q-item
            clickable
            v-ripple
            v-if="!pwaIsInstalled && browserType() === 'chrome'"
            @click="installApp"
          >
            <q-item-section avatar>
      ...
      <div class="bg-grey-8 text-white q-pa-md absolute-top area-height">
        <q-item-section avatar class="row">
          <div v-if="userStore.getEmail">
            <q-icon size="20px" name="verified_user" />
            <span class="q-pl-sm"
              >{{ userStore.getFirstName }} {{ userStore.getLastName }}</span
            >
          </div>
          <div v-else>
