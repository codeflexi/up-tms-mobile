<template>
  <div id="Directs" class="font-prompt-300">
    <q-toolbar class="bg-grey-10 text-white">
      ...
      <q-toolbar-title>Pick-Up Lists</q-toolbar-title>
    </q-toolbar>
    ...
    <q-scroll-area v-else class="list">
      ...
      <router-link
        ...
        :key="pick._id"
        :to="`/pick/add-pick/${pick._id}`"
        class="warehouse"
        style="text-decoration: none; color: black"
      >
        <q-list
          bordered
          class="flex column justify-center q-mt-sm rounded-borders shadow-3 bg-yellow-1"
        >
          <q-item-label
            class="text-weight-bold rounded-borders text-white bg-orange-8"
            header
          >
            <div class="q-pb-xs">
              {{ pick.company?.name }}
          ...
          <q-item>
            <q-item-section side>
              <q-icon color="yellow" name="inventory_2" size="lg" />
            </q-item-section>
            <q-btn class="q-mx-xs q-text-center" size="sm" color="green"
